...
def _ensure_h264(video_path: Path) -> None:
    """Transcode annotated video to browser-compatible H.264 (yuv420p) with faststart."""
    ...
    try:
        ...
        temp_path = video_path.with_name(f"{video_path.stem}_h264.mp4")
        out = av.open(str(temp_path), mode="w", format="mp4")
        out_video = out.add_stream("h264", rate=stream.average_rate or 10)
        out_video.width = stream.width
        out_video.height = stream.height
        out_video.pix_fmt = "yuv420p"
        out_video.options = {"crf": "22", "preset": "veryfast", "movflags": "+faststart"}
        ...
        print(f"[WARN] H.264 transcoding skipped/failed: {e}")
