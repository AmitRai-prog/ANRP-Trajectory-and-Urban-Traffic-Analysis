...
export function renderDashboard(container, results, { onNewUpload }) {
  ...
  const incidents = results.incidents || analytics.incidents || [];
        ...
        <div class="incident-monitor-card animate-in animate-in-delay-1" style="margin-bottom: 28px;">
          <div class="incident-monitor-header">
            <div style="display: flex; align-items: center; gap: 10px;">
              ...
              <span class="incident-count-tag">${incidents.length} Event${incidents.length === 1 ? '' : 's'}</span>
            </div>
            ...
            ${
              incidents.length === 0
                ...
                : incidents.map(inc => {
                    ...
                    const tracksStr = (inc.involved_tracks || []).map(t => `#${t.track_id}`).join(', ') || '#--';
                    const confPct = Math.round((inc.confidence || 0.8) * 100);
                    return `
                      <div class="incident-item-card">
                        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 8px;">
                          <div style="display: flex; align-items: center; gap: 8px;">
                            <span class="incident-badge-severity ${sev}">${sev.toUpperCase()}</span>
                            <span class="mono" style="font-weight: 700; font-size: 13px; color: var(--text-primary);">${inc.incident_id}</span>
                          </div>
                          <span class="incident-badge-status ${inc.status || 'open'} inc-status-${inc.incident_id}">
                        ...
                        <div style="font-size: 12px; color: var(--text-muted); margin-bottom: 12px; line-height: 1.5;">
                          <div>Involved Vehicles: <strong class="mono" style="color: var(--accent-blue);">${tracksStr}</strong> • Time: <span class="mono">${inc.timestamp_s}s</span></div>
                          <div>Confidence: <strong style="color: var(--text-primary);">${confPct}%</strong></div>
                        </div>
